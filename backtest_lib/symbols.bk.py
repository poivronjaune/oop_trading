...
class Symbols:
    ...
    def extract_symbol_lines_from_html_contente(self):
        if self.html is None:
            self._no_symbols_found()
            return

        bs4_soup = BeautifulSoup(self.html, "html.parser")
        first_line_to_parse = self._get_first_line_of_data_to_parse_from(bs4_soup)
        ...
        ) = self._extract_symbol_lines_as_strings(first_line_to_parse)

        listed_df = self._convert_strings_list_to_dataframe(listed_symbols_as_strings)

        self.symbols_df = listed_df  # TODO: merge listed symbols and delisted symbols as one Dataframe

    def convert_symbol_lines_to_dataframe(self):
        pass

    def _get_first_line_of_data_to_parse_from(self, bs4_soup):
        try:
    ...
    def _extract_symbol_lines_as_strings(self, lines_to_parse):
        TEXT_MARKER = (
    ...
    def _convert_strings_list_to_dataframe(self, symbols_str_list):
